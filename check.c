...
/*   Updated: 2019/01/17 11:48:22 by bprunevi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++j <= 3 && (i = -1) && (rtn[5][j] = '.'))
		while (++i <= 3)
			if (rtn[5][j] != '#' && rtn[i][j] == '#' && ++tt.l)
				rtn[5][j] = '#';
	i = -1;
	tt.str = malloc(sizeof(char *) * tt.h);
	while (++i <= 3 && (j = -1))
		if (rtn[4][i] == '#' && (tt.str[x / tt.l] = malloc(tt.l)))
			while (++j <= 3)
				if (rtn[5][j] == '#' && ++x)
					tt.str[(x - 1) / tt.l][(x - 1) % tt.l] = rtn[i][j];

	return (tt);
}

char	bin_tetri(char *str)
{
	char	rtn[6][4];
	int		i;
	int		j;
